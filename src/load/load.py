...
from db.db import create_db_table
import os
...
def insert_forecasts_to_db(dfs: List[pd.DataFrame]) -> None:
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(
                text("SELECT City, forecast_time FROM weather_forecast")
            )
        ...
        for df in dfs:
            ...
            df_filtered = df[
                ~df.apply(
                    lambda row: (row["City"], row["forecast_time"]) in existing_set,
                    axis=1,
                )
            ]
        ...
        logger.error(f"Error inserting data into RDS: {str(e)}")
        raise
